...
ll createHashValue(string str, int n){ // O(n)
	ll result = 0;
	for(int i = 0; i < n; i++){
...
bool checkEquality(string str1, string str2, int start1, int end1, int start2, int end2){
...
int main(int argc, char const *argv[])
{
	...
	for(int i = 0; i < str.length() - pat.length(); i++){
		if((patHash == strHash) && checkEquality(str, pat, i, i+pat.length()-1, 0, pat.length()-1)){
			cout << i << endl;
			return 0;
		}
		if(i < (str.length()- pat.length())){
			strHash = recalculateHash(str, i, i+pat.length()-1, strHash, pat.length());
			cout << strHash << endl;
		}
	}
