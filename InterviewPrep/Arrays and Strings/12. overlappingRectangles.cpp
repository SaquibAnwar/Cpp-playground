...
// Input Format
// First line contains an 4 integers i.e x1, y1, x2, y2 for rectangle 1, 
// similarly second line contains coordinates for rectangle 2.
...
bool isOverlapping(int x0, int y0, int x1, int y1, int a0, int b0, int a1, int b1)
{
    if(!(x1 <= a0 || y0 >= b1 || a1 <= x0 || b0 >= y1))
        return true;
...
int main()
{
    ...
    cin >> x0 >> y0 >> x1 >> y1;
    cin >> a0 >> b0 >> a1>> b1;
    cout << isOverlapping(x0, y0, x1, y1, a0, b0, a1, b1) << endl;
}